...
## Add lasso regularization (multiclass)for comparison
```{r Analysis Part One Tuned Parameters Boosting model}
...
In the first analysis, we classified the videos into 17 specific categories; perhaps, we can reduce the number of categories from 17 specific categories to 4 broader, more encompassing categories and examine if we can obtain a better result.
...
## Add Tuned lasso model with reduced categories for comparison
```{r Analysis Part Two tuned lasso}
set.seed(20211213)
...
## Add Tuned lasso model with sentiment score for comparison
```{r Analysis Part Three tuned lasso}
set.seed(20211213)
...
conf_mat_tuned_lasso_sentiment
...
### Conclusion
   The raw data we used for this project is the most popular YouTube videos in the U.S, and the target variable we investigated is the category of each video. We firstly use Random Forest model with raw data, which has 17 categories, and compared the accuracy of tuned Random Forest model and tuned Lasso model. Then we tried to reclassify 17 categories into 4 broader categories, and then compared RF and Lasso models as well. Last part, we added sentiment score of the video description as another explanatory variable in our models. We tuned RF and Lasso model and concluded that among all three analysis parts, RF model is better than Lasso model regarding the accuracy. By adding sentiment score and using broader categories, the accuracy of our model increased.