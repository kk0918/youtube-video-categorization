...
all_videos_sentiment_df <- mutate(all_videos_broad_df,
                                  ...
                                  description = str_remove_all(description, pattern = "<.*?>") # Remove all text between < > 
                                  )

# Note - runs super slow for large DF
...
sentiment_rf_model <- rand_forest() %>%
  set_engine("randomForest",
             num.threads = parallel::detectCores(), 
             importance = TRUE, 
             verbose = TRUE) %>% 
...
sentiment_rf_fit <- fit(sentiment_rf_wf, sentiment_training)
